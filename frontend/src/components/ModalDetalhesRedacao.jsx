...
export default function ModalDetalhesRedacao({ redacao, onClose, onUpdated }) {
  ...
  const renderMinimalistOfficialSheet = () => (
    <div id="minimalist-pdf-document" className="w-[720px] bg-[#ffffff] text-[#111111] p-6 space-y-5 font-sans text-xs border border-[#d0d0d0] rounded-lg">
      ...
      <div className="space-y-4">
        ...
        <div className="border-b-2 border-[#111111] pb-2 flex justify-between items-start" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          <div className="text-right font-mono text-[9.5px] text-[#444444] border-l border-[#d0d0d0] pl-3">
        ...
        <div className="grid grid-cols-3 gap-3 border border-[#111111] p-3 rounded font-mono text-[10px]" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          <div className="col-span-2 space-y-1">
            <div><span className="text-[#666666] uppercase text-[9px] block font-sans font-bold">Estudante:</span> <strong className="text-sm text-[#111111] font-sans">{data.aluno || redacao.nome_aluno || 'Estudante Não Identificado'}</strong></div>
            <div className="flex gap-4 text-[10.5px] pt-1">
          ...
          <div className="border-l border-[#111111] pl-3 text-center flex flex-col justify-center bg-[#fcfcfc] p-1.5">
        ...
        <div className="space-y-1.5" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          <table className="w-full text-left border-collapse text-[10px] border border-[#111111]">
            <thead>
              <tr className="bg-[#f2f2f2] border-b border-[#111111] font-mono text-[9.5px] uppercase">
              ...
              {enemCompetenciasMap.map(({ key, title }) => {
                ...
                return (
                  <tr key={key} className="align-top" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    ...
                    <td className="p-2 border-r border-[#111111] font-mono text-[9px] italic bg-[#fafafa]">
                      {comp.citacao_texto ? `"${comp.citacao_texto}"` : '—'}
                    </td>
                    <td className="p-2 leading-tight text-[#222222]">
                      {comp.justificativa}
                    </td>
                  </tr>
                );
        ...
        {/* GAP VISUAL ENTRE SEÇÃO 1 E SEÇÃO 2 (Espaçamento limpo) */}
        <div className="py-2" />
                ...
                {siseduDiscursivaMap.map(({ key, title }) => {
                  ...
                  return (
                    <div key={key} className="border-b border-[#eeeeee] pb-1 last:border-b-0">
                      ...
                      <p className="text-[#444444] leading-tight text-[9px] mt-0.5">{item.justificativa}</p>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="space-y-2 flex-1" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          <h2 className="text-xs font-bold uppercase text-[#111111] tracking-wider border-b border-[#111111] pb-1 font-mono">
            3. TRANSCRIÇÃO INTEGRAL DO TEXTO DA REDAÇÃO (VERBATIM)
          </h2>
          <div className="p-4 border border-[#111111] bg-[#fafafa] font-mono text-[10px] leading-relaxed whitespace-pre-wrap text-[#111111] min-h-[380px] rounded">
            {fullTextContent}
          </div>
        </div>

        {/* Footer & Teacher Signature Box */}
        <div className="pt-8 flex justify-between items-end text-[10px] font-mono border-t border-[#111111] text-[#444444]" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          <div>
          ...
          <div className="text-center w-64 border-t border-[#111111] pt-2">
